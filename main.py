...
def main():
    ...
    while True:
        ...
        try:
            ...
            if user_input == '1':
                user_input = input("1. SavingsAccount\n2. CheckingAccount\nAccount Type(1-2): ")
                if user_input == '1':
                    name_input = input("Name: ")
                    balance_input = input('Starting Balance: ')
                    print(f"""Account successfully created!
                        {bank.create_account("SavingsAccount", name_input, balance_input, savings_rate)}
                        """)  
                elif user_input == '2':
                    name_input = input("Name: ")
                    balance_input = input('Starting Balance: ')
                    print(f"""Account successfully created!
                        {bank.create_account("CheckingAccount", name_input, balance_input, overdraft_limit)}
                        """)
                ...
                if user_action == '1':
                    ...
                    amount_input = input('Amount to be transfered: ')
                ...
                print(f"Hello {portfolio_account._owner}")
                if user_input == '1':
                    ticker_input = input("Symbol: ")
                    share_input = int(input("Shares: "))
                    cost_input = Decimal(str(input('Cost: ')))
                    portfolio_account.add_position(ticker_input,share_input, cost_input)
